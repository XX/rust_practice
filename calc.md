...
Если установку `token_value` в значение `TokenValue::Number` или `TokenValue::Name` удобно делать в ветках `match` как только мы встретили подходящий символ для числа или имени соответственно, то проверку на окончание вычитывания значения `token_value` удобнее располагать в начале цикла, до того, как будет осуществляться проверка `match` на другие токены, и осуществляться она должна только в том случае, если не был вычитан очередной символ текущего значения `token_value`.

Имейте ввиду, что для случая `TokenValue::Name` изначально проверяется совпадение символа с одной из алфавитных букв или с символом подчеркивания, а затем к ним присоединяется также возможность совпадения с цифрой, так как имена не могут начинаться с цифр, но они могут их в себе содержать.

Для случая `TokenValue::Number`, при преобразовании его значения в значение `Token::Number`, вам понадобится конвертировать строку в число. Найдите способ это сделать.
